...
class LoanRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAllLoansOnce(): List<LoanEntity> = loanDao.getAllLoansList()

    override fun getAllLoans(): Flow<List<LoanEntity>> = loanDao.getAllLoans()

    override fun getLoansByStatus(status: String): Flow<List<LoanEntity>> = loanDao.getLoansByStatus(status)
    ...
    override fun getTotalBorrowedAmount(): Flow<Double> = loanDao.getTotalBorrowedAmount().map { it ?: 0.0 }

    override fun getTotalLentAmount(): Flow<Double> = loanDao.getTotalLentAmount().map { it ?: 0.0 }

    override fun getEventsForLoan(loanId: Long): Flow<List<LoanEventEntity>> = loanEventDao.getEventsForLoan(loanId)

    override suspend fun insertLoanEvent(event: LoanEventEntity): Long = loanEventDao.insert(event)
}
