...
data class DashboardHubState(
    ...
    val isLedgerConsistent: Boolean = true // Added for Integrity Indicator
)
    ...
    init {
        viewModelScope.launch {
            combine(
                ...
                _currentYear,
                _selectedDate
            ) { wallets, month, year, date ->
                updateTotals(wallets, month, year, date)
            }.collect()
        }
    }

    private suspend fun updateTotals(wallets: List<WalletEntity>, month: YearMonth, year: Year, date: LocalDate) {
        // Monthly Totals
        ...
        // Net Worth Logic (Section 7.0)
        val assets = wallets.filter { it.type != "CREDIT" && it.type != "CREDIT_CARD" }.sumOf { it.currentBalance }
        val liabilities = wallets.filter { it.type == "CREDIT" || it.type == "CREDIT_CARD" }.sumOf { it.currentBalance }
        ...
        _hubState.value = DashboardHubState(
            ...
            isLedgerConsistent = mismatches.isEmpty()
        )
    }
